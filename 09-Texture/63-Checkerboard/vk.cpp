...
// Global Variables
const char* gpszAppName = "AMK_VulkanXWindowsApp : Checkerboard";
...
// For checkerboard
byte checkerBoard[CHECKERBOARD_HEIGHT][CHECKERBOARD_WIDTH][4];
...
void uninitialize(void) {
    ...
    if(uniformData_oblique.vkDeviceMemory) {
        ...
        fprintf(fptr, "uninitialize(): vkFreeMemory() Succeed for Uniform Buffer for Oblique!\n");
    ...
    if(uniformData_oblique.vkBuffer) {
        ...
        fprintf(fptr, "uninitialize(): vkDestroyBuffer() Succeed for Uniform Buffer for Oblique!\n");
    ...
    if(uniformData_straight.vkDeviceMemory) {
        ...
        fprintf(fptr, "uninitialize(): vkFreeMemory() Succeed for Uniform Buffer for Straight!\n");
    ...
    if(uniformData_straight.vkBuffer) {
        ...
        fprintf(fptr, "uninitialize(): vkDestroyBuffer() Succeed for Uniform Buffer for Straight!\n");
...
VkResult createTexture() {
    ...
    int texture_width = CHECKERBOARD_WIDTH, texture_height = CHECKERBOARD_HEIGHT;

    image_data = (uint8_t*)checkerBoard;

    VkDeviceSize image_size = texture_width * texture_height * 4; // 4 channels (RGBA)

    fprintf(fptr, "createTexture(): Texture Image Loaded Successfully! Width: %d, Height: %d, Size: %llu bytes\n", 
        texture_width, texture_height, (unsigned long long)image_size);
    ...
    // As Copying of Image Data is done, we can free Image Data
    image_data = NULL;
    fprintf(fptr, "createTexture(): Image Data Copied to Staging Buffer Successful!.\n");
    fflush(fptr);
    ...
    vkSamplerCreateInfo.magFilter = VK_FILTER_NEAREST; // Linear Filtering
    vkSamplerCreateInfo.minFilter = VK_FILTER_NEAREST;
    vkSamplerCreateInfo.mipmapMode = VK_SAMPLER_MIPMAP_MODE_NEAREST; // Linear Mipmapping
...
VkResult createUniformBuffer (void) {
    ...
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createUniformBuffer(): vkCreateBuffer() Failed for Straight Uniform Buffer!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createUniformBuffer(): vkCreateBuffer() Successful for Straight Uniform Buffer!.\n");
    }
    ...
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createUniformBuffer(): vkAllocateMemory() Failed for Straight Uniform Buffer!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createUniformBuffer(): vkAllocateMemory() Successful for Straight Uniform Buffer!.\n");
    }

    // Step 10
    vkResult = vkBindBufferMemory(vkDevice, uniformData_straight.vkBuffer, uniformData_straight.vkDeviceMemory, 0);
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createUniformBuffer(): vkBindBufferMemory() Failed for Straight Uniform Buffer!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createUniformBuffer(): vkBindBufferMemory() Successful for Straight Uniform Buffer!.\n");
    }

    // For oblique uniform buffer
    ...
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createUniformBuffer(): vkCreateBuffer() Failed for Oblique Uniform Buffer!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createUniformBuffer(): vkCreateBuffer() Successful for Oblique Uniform Buffer!.\n");
    }
    ...
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createUniformBuffer(): vkAllocateMemory() Failed for Oblique Uniform Buffer!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createUniformBuffer(): vkAllocateMemory() Successful for Oblique Uniform Buffer!.\n");
    }

    // Step 10
    vkResult = vkBindBufferMemory(vkDevice, uniformData_oblique.vkBuffer, uniformData_oblique.vkDeviceMemory, 0);
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createUniformBuffer(): vkBindBufferMemory() Failed for Oblique Uniform Buffer!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createUniformBuffer(): vkBindBufferMemory() Successful for Oblique Uniform Buffer!.\n");
    }
...
VkResult createDescriptorSet(void) {
    ...
    // vkDescriptorSet Straight
    // Allocate Descriptor Set 
    vkResult = vkAllocateDescriptorSets(vkDevice, &vkDescriptorSetAllocateInfo, &vkDescriptorSet_straight);
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createDescriptorSet(): vkAllocateDescriptorSets() Failed for straight descriptor set!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createDescriptorSet(): vkAllocateDescriptorSets() Successful for straight descriptor set!.\n");
    }
    ...
    // for mvp uniform
    vkDescriptorBufferInfo.buffer = uniformData_straight.vkBuffer; // this is the buffer we want to use as uniform
    ...
    vkWriteDescriptorSet_array[0].dstSet = vkDescriptorSet_straight; // this is the descriptor set we want to update
    ...
    vkWriteDescriptorSet_array[1].dstSet = vkDescriptorSet_straight; // this is the descriptor set we want to update
    ...
    fprintf(fptr, "createDescriptorSet(): vkUpdateDescriptorSets() Successful for straight descriptor set!.\n");

    // For vkDescriptorSet_oblique
    memset((void*)&vkDescriptorSetAllocateInfo, 0, sizeof(VkDescriptorSetAllocateInfo));
    ...
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createDescriptorSet(): vkAllocateDescriptorSets() Failed for oblique descriptor set!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createDescriptorSet(): vkAllocateDescriptorSets() Successful for oblique descriptor set!.\n");
    }
    ...
    // for mvp uniform
    vkDescriptorBufferInfo.buffer = uniformData_oblique.vkBuffer; // this is the buffer we want to use as uniform
    vkDescriptorBufferInfo.offset = 0; // offset is 0
    vkDescriptorBufferInfo.range = sizeof(struct MyUniformData); // range is size of uniform
    ...
    vkWriteDescriptorSet_array[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    vkWriteDescriptorSet_array[0].pNext = NULL;
    vkWriteDescriptorSet_array[0].dstSet = vkDescriptorSet_oblique; // this is the descriptor set we want to update
    vkWriteDescriptorSet_array[0].dstArrayElement = 0; // we have only one descriptor set, so array element is 0
    vkWriteDescriptorSet_array[0].descriptorCount = 1; // we are only gonna write one descriptor set
    vkWriteDescriptorSet_array[0].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER; 
    vkWriteDescriptorSet_array[0].pBufferInfo = &vkDescriptorBufferInfo;
    ...
    vkWriteDescriptorSet_array[1].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    vkWriteDescriptorSet_array[1].pNext = NULL;
    vkWriteDescriptorSet_array[1].dstSet = vkDescriptorSet_oblique; // this is the descriptor set we want to update
    vkWriteDescriptorSet_array[1].dstArrayElement = 0; // we have only one descriptor set, so array element is 0
    vkWriteDescriptorSet_array[1].descriptorCount = 1; // we are only gonna write one descriptor set
    vkWriteDescriptorSet_array[1].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER; 
    vkWriteDescriptorSet_array[1].pBufferInfo = NULL;
    ...
    fprintf(fptr, "createDescriptorSet(): vkUpdateDescriptorSets() Successful for oblique descriptor set!.\n");
...
VkResult buildCommandBuffers(void) {
    ...
    for(uint32_t i = 0; i < swapchainImageCount; i++) {
        ...
        vkCmdBindDescriptorSets(
            ...
            vkPipelineLayout,
            0, 1,
            &vkDescriptorSet_straight, // this is the descriptor set we want to bind
            0, NULL
        );
        ...
        // Here we should call vulkan drawing functions! [ For Straight Quad ]
        ...
        vkCmdBindDescriptorSets(
            ...
            vkPipelineLayout,
            0, 1,
            &vkDescriptorSet_oblique, // this is the descriptor set we want to bind
            0, NULL
        );

        // Here we should call vulkan drawing functions! [ For Oblique Quad ]
        vkCmdDraw(vkCommandBuffer_array[i], 6, 1, 0, 0);

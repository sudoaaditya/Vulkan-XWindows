...
// Global Variables
const char* gpszAppName = "AMK_VulkanXWindowsApp : Smiley Texture";
...
int main(int argc, char *argv[]) {
    ...
    void update(void);
    void updateTexCoordBuffer(const float *);
            ...
            switch(event.type) {
                ...
                case KeyPress:
                    ...
                    switch(keys[0]) {
                        ...
                        case '1':
                            updateTexCoordBuffer(texCoord0);
                            break;

                        case '2':
                            updateTexCoordBuffer(texCoord1);
                            break;

                        case '3':
                            updateTexCoordBuffer(texCoord3);
                            break;
...
VkResult createVertexBuffer(void) {
    ...
    vkBufferCreateInfo.pNext = NULL;
    vkBufferCreateInfo.flags = 0; // No flags, Valid Flags are used in scattered buffer
    vkBufferCreateInfo.size = sizeof(texCoord0);
    vkBufferCreateInfo.usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
...
void updateTexCoordBuffer(const float *texCoord) {
    void *data = NULL;

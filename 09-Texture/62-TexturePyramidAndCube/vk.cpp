...
// Global Variables
const char* gpszAppName = "AMK_VulkanXWindowsApp : Textured Pyramid & Cube";
...
// Position & Color for Pyramid
VertexData vertexData_pyramid_position;
VertexData vertexData_pyramid_texcoord;
// Position & Color for Cube
VertexData vertexData_cube_position;
VertexData vertexData_cube_texcoord;
...
// Texture Related Variables [ Cube & Pyramid ]
...
// For Rotation
float fRotAngle_pyramid = 0.0f;
float fRotAngle_cube = 0.0f;
...
VkResult initialize(void) {
    ...
    // Create Texture
    vkResult = createTexture("Vijay_Kundali.png", &vkImage_texture_cube, &vkDeviceMemory_texture_cube, &vkImageView_texture_cube);
    if(vkResult != VK_SUCCESS) {
    ...
    vkResult = createTexture("Stone.png", &vkImage_texture_pyramid, &vkDeviceMemory_texture_pyramid, &vkImageView_texture_pyramid);
    if(vkResult != VK_SUCCESS) {
...
void uninitialize(void) {
    ...
    // Destroy Uniform Buffer Pyramid
    ...
    // Destroy Vertex Buffer Color for Pyramid
    if(vertexData_pyramid_texcoord.vkDeviceMemory) {
        vkFreeMemory(vkDevice, vertexData_pyramid_texcoord.vkDeviceMemory, NULL);
        fprintf(fptr, "uninitialize(): vkFreeMemory() Succeed for Vertex Buffer for Color!\n");
        vertexData_pyramid_texcoord.vkDeviceMemory = VK_NULL_HANDLE;
    }

    if(vertexData_pyramid_texcoord.vkBuffer) {
        vkDestroyBuffer(vkDevice, vertexData_pyramid_texcoord.vkBuffer, NULL);
        fprintf(fptr, "uninitialize(): vkDestroyBuffer() Succeed for Vertex Buffer for Color!\n");
        vertexData_pyramid_texcoord.vkBuffer = VK_NULL_HANDLE;
    }

    // Destroy Vertex Buffer Position Pyramid
    if(vertexData_pyramid_position.vkDeviceMemory) {
        vkFreeMemory(vkDevice, vertexData_pyramid_position.vkDeviceMemory, NULL);
        fprintf(fptr, "uninitialize(): vkFreeMemory() Succeed for Vertex Buffer for Position!\n");
        vertexData_pyramid_position.vkDeviceMemory = VK_NULL_HANDLE;
    }

    if(vertexData_pyramid_position.vkBuffer) {
        vkDestroyBuffer(vkDevice, vertexData_pyramid_position.vkBuffer, NULL);
        fprintf(fptr, "uninitialize(): vkDestroyBuffer() Succeed for Vertex Buffer for Position!\n");
        vertexData_pyramid_position.vkBuffer = VK_NULL_HANDLE;
    }

     // Destroy Vertex Buffer Color Cube
    if(vertexData_cube_texcoord.vkDeviceMemory) {
        vkFreeMemory(vkDevice, vertexData_cube_texcoord.vkDeviceMemory, NULL);
        fprintf(fptr, "uninitialize(): vkFreeMemory() Succeed for Vertex Buffer for Color!\n");
        vertexData_cube_texcoord.vkDeviceMemory = VK_NULL_HANDLE;
    }

    if(vertexData_cube_texcoord.vkBuffer) {
        vkDestroyBuffer(vkDevice, vertexData_cube_texcoord.vkBuffer, NULL);
        fprintf(fptr, "uninitialize(): vkDestroyBuffer() Succeed for Vertex Buffer for Color!\n");
        vertexData_cube_texcoord.vkBuffer = VK_NULL_HANDLE;
    }

    // Destroy Vertex Buffer Position Cube
    if(vertexData_cube_position.vkDeviceMemory) {
        vkFreeMemory(vkDevice, vertexData_cube_position.vkDeviceMemory, NULL);
        fprintf(fptr, "uninitialize(): vkFreeMemory() Succeed for Vertex Buffer for Position!\n");
        vertexData_cube_position.vkDeviceMemory = VK_NULL_HANDLE;
    }

    if(vertexData_cube_position.vkBuffer) {
        vkDestroyBuffer(vkDevice, vertexData_cube_position.vkBuffer, NULL);
        fprintf(fptr, "uninitialize(): vkDestroyBuffer() Succeed for Vertex Buffer for Position!\n");
        vertexData_cube_position.vkBuffer = VK_NULL_HANDLE;
    }
...
void update(void) {
    ...
    fRotAngle_pyramid += 0.05f;
    if(fRotAngle_pyramid >= 360.0f) {
        fRotAngle_pyramid = 0.0f;
    }

    fRotAngle_cube += 0.05f;
    if(fRotAngle_cube >= 360.0f) {
        fRotAngle_cube = 0.0f;
    }
}
...
VkResult createVertexBuffer(void) {
    ...
    // Step 2
    memset((void*)&vertexData_pyramid_position, 0, sizeof(VertexData));
    ...
    // Setp 4
    vkResult = vkCreateBuffer(vkDevice, &vkBufferCreateInfo, NULL, &vertexData_pyramid_position.vkBuffer);
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createVertexBuffer(): vkCreateBuffer() Failed for Pyramid Position!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createVertexBuffer(): vkCreateBuffer() Successful for Pyramid Position!.\n");
    }
    ...
    vkGetBufferMemoryRequirements(vkDevice, vertexData_pyramid_position.vkBuffer, &vkMemoryRequirements);
    ...
    //Setp 9
    vkResult = vkAllocateMemory(vkDevice, &vkMemoryAllocateInfo, NULL, &vertexData_pyramid_position.vkDeviceMemory);
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createVertexBuffer(): vkAllocateMemory() Failed for Pyramid Position!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createVertexBuffer(): vkAllocateMemory() Successful for Pyramid Position!.\n");
    }

    // Step 10
    vkResult = vkBindBufferMemory(vkDevice, vertexData_pyramid_position.vkBuffer, vertexData_pyramid_position.vkDeviceMemory, 0);
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createVertexBuffer(): vkBindBufferMemory() Failed for Pyramid Position!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createVertexBuffer(): vkBindBufferMemory() Successful for Pyramid Position!.\n");
    }
    ...
    vkResult = vkMapMemory(
        vkDevice,
        vertexData_pyramid_position.vkDeviceMemory,
        0,
    ...
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createVertexBuffer(): vkMapMemory() Failed for Pyramid Position!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createVertexBuffer(): vkMapMemory() Successful for Pyramid Position!.\n");
    }
    ...
    // Step 13
    vkUnmapMemory(vkDevice, vertexData_pyramid_position.vkDeviceMemory);

    fprintf(fptr, "\n");

    // VertexData for Triangle Color
    memset((void*)&vertexData_pyramid_texcoord, 0, sizeof(VertexData));
    ...
    // Setp 4
    vkResult = vkCreateBuffer(vkDevice, &vkBufferCreateInfo, NULL, &vertexData_pyramid_texcoord.vkBuffer);
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createVertexBuffer(): vkCreateBuffer() Failed for Pyramid Color!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createVertexBuffer(): vkCreateBuffer() Successful for Pyramid Color!.\n");
    }

    // Step 5
    memset((void*)&vkMemoryRequirements, 0, sizeof(VkMemoryRequirements));

    vkGetBufferMemoryRequirements(vkDevice, vertexData_pyramid_texcoord.vkBuffer, &vkMemoryRequirements);
    ...
    //Setp 9
    vkResult = vkAllocateMemory(vkDevice, &vkMemoryAllocateInfo, NULL, &vertexData_pyramid_texcoord.vkDeviceMemory);
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createVertexBuffer(): vkAllocateMemory() Failed for Pyramid Color!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createVertexBuffer(): vkAllocateMemory() Successful for Pyramid Color!.\n");
    }

    // Step 10
    vkResult = vkBindBufferMemory(vkDevice, vertexData_pyramid_texcoord.vkBuffer, vertexData_pyramid_texcoord.vkDeviceMemory, 0);
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createVertexBuffer(): vkBindBufferMemory() Failed for Pyramid Color!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createVertexBuffer(): vkBindBufferMemory() Successful for Pyramid Color!.\n");
    }
    ...
    vkResult = vkMapMemory(
        vkDevice,
        vertexData_pyramid_texcoord.vkDeviceMemory,
        0,
        vkMemoryAllocateInfo.allocationSize,
        0,
        &data
    ...
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createVertexBuffer(): vkMapMemory() Failed for Pyramid Color!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createVertexBuffer(): vkMapMemory() Successful for Pyramid Color!.\n");
    }
    ...
    // Step 13
    vkUnmapMemory(vkDevice, vertexData_pyramid_texcoord.vkDeviceMemory);
    ...
    // VertexData for Triangle Position
    // Step 2
    memset((void*)&vertexData_cube_position, 0, sizeof(VertexData));
    ...
    // Setp 4
    vkResult = vkCreateBuffer(vkDevice, &vkBufferCreateInfo, NULL, &vertexData_cube_position.vkBuffer);
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createVertexBuffer(): vkCreateBuffer() Failed for Cube Position!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createVertexBuffer(): vkCreateBuffer() Successful for Cube Position!.\n");
    }

    // Step 5
    memset((void*)&vkMemoryRequirements, 0, sizeof(VkMemoryRequirements));

    vkGetBufferMemoryRequirements(vkDevice, vertexData_cube_position.vkBuffer, &vkMemoryRequirements);
    ...
    //Setp 9
    vkResult = vkAllocateMemory(vkDevice, &vkMemoryAllocateInfo, NULL, &vertexData_cube_position.vkDeviceMemory);
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createVertexBuffer(): vkAllocateMemory() Failed for Cube Position!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createVertexBuffer(): vkAllocateMemory() Successful for Cube Position!.\n");
    }

    // Step 10
    vkResult = vkBindBufferMemory(vkDevice, vertexData_cube_position.vkBuffer, vertexData_cube_position.vkDeviceMemory, 0);
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createVertexBuffer(): vkBindBufferMemory() Failed for Cube Position!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createVertexBuffer(): vkBindBufferMemory() Successful for Cube Position!.\n");
    }
    ...
    vkResult = vkMapMemory(
        vkDevice,
        vertexData_cube_position.vkDeviceMemory,
        0,
        vkMemoryAllocateInfo.allocationSize,
        0,
        &data
    ...
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createVertexBuffer(): vkMapMemory() Failed for Cube Position!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createVertexBuffer(): vkMapMemory() Successful for Cube Position!.\n");
    }
    ...
    // Step 13
    vkUnmapMemory(vkDevice, vertexData_cube_position.vkDeviceMemory);

    fprintf(fptr, "\n");

    // VertexData for Triangle Color
    memset((void*)&vertexData_cube_texcoord, 0, sizeof(VertexData));
    ...
    // Setp 4
    vkResult = vkCreateBuffer(vkDevice, &vkBufferCreateInfo, NULL, &vertexData_cube_texcoord.vkBuffer);
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createVertexBuffer(): vkCreateBuffer() Failed for Cube Color!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createVertexBuffer(): vkCreateBuffer() Successful for Cube Color!.\n");
    }

    // Step 5
    memset((void*)&vkMemoryRequirements, 0, sizeof(VkMemoryRequirements));

    vkGetBufferMemoryRequirements(vkDevice, vertexData_cube_texcoord.vkBuffer, &vkMemoryRequirements);
    ...
    //Setp 9
    vkResult = vkAllocateMemory(vkDevice, &vkMemoryAllocateInfo, NULL, &vertexData_cube_texcoord.vkDeviceMemory);
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createVertexBuffer(): vkAllocateMemory() Failed for Cube Color!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createVertexBuffer(): vkAllocateMemory() Successful for Cube Color!.\n");
    }

    // Step 10
    vkResult = vkBindBufferMemory(vkDevice, vertexData_cube_texcoord.vkBuffer, vertexData_cube_texcoord.vkDeviceMemory, 0);
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createVertexBuffer(): vkBindBufferMemory() Failed for Cube Color!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createVertexBuffer(): vkBindBufferMemory() Successful for Cube Color!.\n");
    }
    ...
    vkResult = vkMapMemory(
        vkDevice,
        vertexData_cube_texcoord.vkDeviceMemory,
        0,
        vkMemoryAllocateInfo.allocationSize,
        0,
        &data
    ...
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createVertexBuffer(): vkMapMemory() Failed for Cube Color!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createVertexBuffer(): vkMapMemory() Successful for Cube Color!.\n");
    }
    ...
    // Step 13
    vkUnmapMemory(vkDevice, vertexData_cube_texcoord.vkDeviceMemory);
...
VkResult createUniformBuffer (void) {
    ...
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createUniformBuffer(): vkCreateBuffer() Failed for Pyramid!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createUniformBuffer(): vkCreateBuffer() Successful for Pyramid!.\n");
    }
    ...
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createUniformBuffer(): vkAllocateMemory() Failed for Pyramid!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createUniformBuffer(): vkAllocateMemory() Successful for Pyramid!.\n");
    }

    // Step 10
    vkResult = vkBindBufferMemory(vkDevice, uniformData_pyramid.vkBuffer, uniformData_pyramid.vkDeviceMemory, 0);
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createUniformBuffer(): vkBindBufferMemory() Failed for Pyramid!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createUniformBuffer(): vkBindBufferMemory() Successful for Pyramid!.\n");
    }
    ...
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createUniformBuffer(): vkCreateBuffer() Failed for Cube!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createUniformBuffer(): vkCreateBuffer() Successful for Cube!.\n");
    }
    ...
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createUniformBuffer(): vkAllocateMemory() Failed for Cube!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createUniformBuffer(): vkAllocateMemory() Successful for Cube!.\n");
    }

    // Step 10
    vkResult = vkBindBufferMemory(vkDevice, uniformData_cube.vkBuffer, uniformData_cube.vkDeviceMemory, 0);
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createUniformBuffer(): vkBindBufferMemory() Failed for Cube!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createUniformBuffer(): vkBindBufferMemory() Successful for Cube!.\n");
    }
...
VkResult updateUniformBuffer(void) {
    ...
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "updateUniformBuffer(): vkMapMemory() Failed for Pyramid!.\n");
        return (vkResult);
    }
    ...
    myUniformData.modelMatrix = translateMat * scaleMat * rotateMat;
    ...
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "updateUniformBuffer(): vkMapMemory() Failed for Cube!.\n");
        return (vkResult);
    }
...
VkResult createDescriptorSet(void) {
    ...
    // for mvp uniform
    vkDescriptorBufferInfo.buffer = uniformData_pyramid.vkBuffer; // this is the buffer we want to use as uniform
    ...
    vkWriteDescriptorSet_array[0].dstSet = vkDescriptorSet_pyramid; // this is the descriptor set we want to update
    ...
    vkWriteDescriptorSet_array[1].dstSet = vkDescriptorSet_pyramid; // this is the descriptor set we want to update
    ...
    fprintf(fptr, "createDescriptorSet(): vkUpdateDescriptorSets() Successful for Pyramid!.\n");
    ...
    // for mvp uniform
    vkDescriptorBufferInfo.buffer = uniformData_cube.vkBuffer; // this is the buffer we want to use as uniform
    vkDescriptorBufferInfo.offset = 0; // offset is 0
    vkDescriptorBufferInfo.range = sizeof(struct MyUniformData); // range is size of uniform
    ...
    vkWriteDescriptorSet_array[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    vkWriteDescriptorSet_array[0].pNext = NULL;
    vkWriteDescriptorSet_array[0].dstSet = vkDescriptorSet_cube; // this is the descriptor set we want to update
    vkWriteDescriptorSet_array[0].dstArrayElement = 0; // we have only one descriptor set, so array element is 0
    vkWriteDescriptorSet_array[0].descriptorCount = 1; // we are only gonna write one descriptor set
    vkWriteDescriptorSet_array[0].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER; 
    vkWriteDescriptorSet_array[0].pBufferInfo = &vkDescriptorBufferInfo;
    ...
    vkWriteDescriptorSet_array[1].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    vkWriteDescriptorSet_array[1].pNext = NULL;
    vkWriteDescriptorSet_array[1].dstSet = vkDescriptorSet_cube; // this is the descriptor set we want to update
    vkWriteDescriptorSet_array[1].dstArrayElement = 0; // we have only one descriptor set, so array element is 0
    vkWriteDescriptorSet_array[1].descriptorCount = 1; // we are only gonna write one descriptor set
    vkWriteDescriptorSet_array[1].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER; 
    vkWriteDescriptorSet_array[1].pBufferInfo = NULL;
    ...
    fprintf(fptr, "createDescriptorSet(): vkUpdateDescriptorSets() Successful for Cube!.\n");
...
VkResult createPipeline(void) {
    ...
    vkVertexInputBindingDescription_array[1].binding = AMK_ATTRIBUTE_TEXCOORD; // 1st binding index for color
...
VkResult buildCommandBuffers(void) {
    ...
    for(uint32_t i = 0; i < swapchainImageCount; i++) {
        ...
        vkCmdBindDescriptorSets(
            ...
            vkPipelineLayout,
            0, 1,
            &vkDescriptorSet_pyramid, // this is the descriptor set we want to bind
            0, NULL
        );
        ...
        vkCmdBindVertexBuffers(
            ...
            &vertexData_pyramid_position.vkBuffer,
        ...
        vkCmdBindVertexBuffers(
            vkCommandBuffer_array[i], 
            AMK_ATTRIBUTE_TEXCOORD, 1,
            &vertexData_pyramid_texcoord.vkBuffer,
        ...
        vkCmdBindDescriptorSets(
            ...
            vkPipelineLayout,
            0, 1,
            &vkDescriptorSet_cube, // this is the descriptor set we want to bind
            0, NULL
        );
        ...
        vkCmdBindVertexBuffers(
            vkCommandBuffer_array[i], 
            AMK_ATTRIBUTE_POSITION, 1,
            &vertexData_cube_position.vkBuffer,
            vkDeviceSize_offset_position_array
        );
        ...
        vkCmdBindVertexBuffers(
            vkCommandBuffer_array[i], 
            AMK_ATTRIBUTE_TEXCOORD, 1,
            &vertexData_cube_texcoord.vkBuffer,
            vkDeviceSize_offset_texcoord_array
        );

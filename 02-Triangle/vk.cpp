...
// Global Variables
const char* gpszAppName = "ARTR : Perspective Triangle";
...
// VK_KHR_SURFACE_EXTENSION_NAME & VK_KHR_XLIB_SURFACE_EXTENSION_NAME & VK_EXT_DEBUG_REPORT_EXTENSION_NAME
...
VkResult fillInstanceExtensionNames (void) {
    ...
    VkBool32 xLibVulkanSurfaceExtensionFound = VK_FALSE;
    ...
    for(uint32_t i = 0; i < instanceExtensionCount; i++) {
        ...
        if(strcmp(instanceExtensionNames_array[i], VK_KHR_XLIB_SURFACE_EXTENSION_NAME) ==  0) {
            xLibVulkanSurfaceExtensionFound = VK_TRUE;
    ...
    if(xLibVulkanSurfaceExtensionFound == VK_FALSE) {
        vkResult = VK_ERROR_INITIALIZATION_FAILED; // return hardcoded failure
        fprintf(fptr, "fillInstanceExtensionNames(): VK_KHR_XLIB_SURFACE_EXTENSION_NAME Not Found!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "fillInstanceExtensionNames(): VK_KHR_XLIB_SURFACE_EXTENSION_NAME Found!.\n");
    }
    ...
    for(uint32_t i = 0; i < physicalDeviceCount; i++) {
        ...
        for(uint32_t j = 0; j < queueCount; j++) {
            if(vkQueueFamilyProperties_array[j].queueFlags & VK_QUEUE_GRAPHICS_BIT
                ...
                fprintf(fptr, "getPhysicalDevice(): Selected Physical Device Index = %d & Graphics Queue Family Index = %d!.\n", i, j);
                bFound = VK_TRUE;
                break;
            }
        }

...
// Global Variables
const char* gpszAppName = "AMK_VulkanXWindowsApp : Tessellation Shader";
...
VkShaderModule vkShaderModule_tessallation_control = VK_NULL_HANDLE;
VkShaderModule vkShaderModule_tessallation_evaluation = VK_NULL_HANDLE;
...
void uninitialize(void) {
    ...
    if(vkShaderModule_tessallation_evaluation) {
        vkDestroyShaderModule(vkDevice, vkShaderModule_tessallation_evaluation, NULL);
        fprintf(fptr, "uninitialize(): vkDestroyShaderModule() Succeed for Tessellation Evalaution Shader!\n");
        vkShaderModule_tessallation_evaluation = VK_NULL_HANDLE;
    }

    if(vkShaderModule_tessallation_control) {
        vkDestroyShaderModule(vkDevice, vkShaderModule_tessallation_control, NULL);
        fprintf(fptr, "uninitialize(): vkDestroyShaderModule() Succeed for Tessellation Control Shader!\n");
        vkShaderModule_tessallation_control = VK_NULL_HANDLE;
    }
...
VkResult updateUniformBuffer(void) {
    ...
    //Update Tessellation Uniforms
    myUniformData.numberOfLineSegments.x = (float)numLineSegments;
    myUniformData.numberOfLineStrips.x = (float)1; // Single Line Strip
    myUniformData.lineColor = glm::vec4(1.0f, 1.0f, 0.0f, 1.0f); // Yellow Color

    char str[255];
    sprintf(str, "AMK_VulkanXWindowsApp : Tessellation Shader : [ Segments :: %d ]", numLineSegments);
...
VkResult createShaders(void) {
    ...
    fp = fopen(szFileName, "rb");
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createShaders(): fopen() failed to open Tessellation Control Shader spir-v file!.\n");
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        return (vkResult);
    } else {
        fprintf(fptr, "createShaders(): fopen() succeed to open Tessellation Control Shader spir-v file!.\n");
    }
    ...
    if(retVal != 1) {
        fprintf(fptr, "createShaders(): fread() failed to read Tessellation Control Shader file!.\n");
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        return (vkResult);
    } else {
        fprintf(fptr, "createShaders(): fread() succeed to read Tessellation Control Shader file!.\n");
    }
    fclose(fp);
    fp = NULL;
    ...
    vkResult = vkCreateShaderModule(vkDevice, &vkShaderModuleCreateInfo, NULL, &vkShaderModule_tessallation_control);
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createShaders(): vkCreateShaderModule() for Tessellation Control Shader Failed!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createShaders(): vkCreateShaderModule() for Tessellation Control Shader Successful!.\n");
    }
    ...
    fprintf(fptr, "createShaders(): Tessellation Control Shader Module Created Successful!.\n");
    ...
    fp = fopen(szFileName, "rb");
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createShaders(): fopen() failed to open Tessellation Evalaution Shader spir-v file!.\n");
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        return (vkResult);
    } else {
        fprintf(fptr, "createShaders(): fopen() succeed to open Tessellation Evalaution Shader spir-v file!.\n");
    }
    ...
    retVal = fread(shaderData, fileSize, 1, fp);
    if(retVal != 1) {
        fprintf(fptr, "createShaders(): fread() failed to read Tessellation Evalaution Shader file!.\n");
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        return (vkResult);
    } else {
        fprintf(fptr, "createShaders(): fread() succeed to read Tessellation Evalaution Shader file!.\n");
    }
    fclose(fp);
    fp = NULL;
    ...
    vkResult = vkCreateShaderModule(vkDevice, &vkShaderModuleCreateInfo, NULL, &vkShaderModule_tessallation_evaluation);
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createShaders(): vkCreateShaderModule() for Tessellation Evalaution Shader Failed!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createShaders(): vkCreateShaderModule() for Tessellation Evalaution Shader Successful!.\n");
    }
    ...
    fprintf(fptr, "createShaders(): Tessellation Evalaution Shader Module Created Successful!.\n");
...
VkResult createDescriptorSetLayout(void) {
    ...
    vkDescriptorSetLayoutBinding.stageFlags = VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT 
                                                | VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT 
                                                | VK_SHADER_STAGE_FRAGMENT_BIT; // this binding will be used in all but vertex shader
...
VkResult createPipeline(void) {
    ...
    vkPipelineShaderStageCreateInfo_array[1].module = vkShaderModule_tessallation_control;
    ...
    vkPipelineShaderStageCreateInfo_array[2].module = vkShaderModule_tessallation_evaluation;
    ...
    vkPipelineShaderStageCreateInfo_array[3].module = vkShaderModule_fragment;
    vkPipelineShaderStageCreateInfo_array[3].pName = "main"; // entry point name
    ...
    vkPipelineTessellationStateCreateInfo.patchControlPoints = 4; // number of control points per patch

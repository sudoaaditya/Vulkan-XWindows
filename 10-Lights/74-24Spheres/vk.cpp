...
// Global Variables
const char* gpszAppName = "AMK_VulkanXWindowsApp : 24 Spheres";
...
VkResult initialize(void) {
    ...
    vkClearColorValue.float32[0] = 0.15f;
    vkClearColorValue.float32[1] = 0.15f;
    vkClearColorValue.float32[2] = 0.15f;
...
VkResult buildCommandBuffers(void) {
    ...
    for(uint32_t i = 0; i < swapchainImageCount; i++) {
        ...
        int xFactor = 5;
        int yFactor = 7;
        float xCellWdith = (float)winWidth / 6.5f; // 4 columns + 2 margins
        float yCellHeight = (float)winHeight / yFactor; // 6 rows + 2 margins

        float xCenter = winWidth / 2.0f;
        float yCenter = winHeight / 2.0f;

        // move viewport to top-left corner
        float xCurrentWidth = xCenter - ((xFactor/2) * xCellWdith); // 3 columns to left of center
        float yCurrentWidth = yCenter - ((yFactor/2) * yCellHeight); // 4 rows above the center
        ...
        for(uint32_t k = 0; k < cols; k++) {
            for(uint32_t j = 0 ; j < rows; j++) {
                ...
                vkViewport.x = xCurrentWidth + (k * xCellWdith);
                vkViewport.y = yCurrentWidth + (j * yCellHeight);
                vkViewport.width = xCellWdith;
                vkViewport.height = yCellHeight;

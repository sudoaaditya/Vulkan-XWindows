...
#include <glslang/SPIRV/GlslangToSpv.h> // this was sufficient til 1.3.28 vulkan version
#include <glslang/Public/ShaderLang.h> // this is required from recent versions!
...
// Global Variables
const char* gpszAppName = "AMK_VulkanXWindowsApp : Staging Vertex Buffer";
...
VkResult createShaders(void) {
    ...
    // Initialize glslLang Procedured=s
    ...
    if(bDone == False) {
        fprintf(fptr, "createShaders(): glsl2Spv() for Vertex Shader Failed!.\n");
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        return (vkResult);
    } else {
        fprintf(fptr, "createShaders(): glsl2Spv() for Vertex Shader Successful!.\n");
    }
    ...
    if(bDone == False) {
        fprintf(fptr, "createShaders(): glsl2Spv() for Fragment Shader Failed!.\n");
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        return (vkResult);
    } else {
        fprintf(fptr, "createShaders(): glsl2Spv() for Fragment Shader Successful!.\n");
    }
...
Bool glsl2Spv(const VkShaderStageFlagBits shaderType, const char* pShader, vector<unsigned int>& spv) {
    // function definations
    EShLanguage findShaderLanguage(const VkShaderStageFlagBits);
    void initBuiltInResources(TBuiltInResource&);

    // code
    //Find Shader Language in the form of EShLanguage
    EShLanguage shaderLanguage = findShaderLanguage(shaderType);
    
    // Create TShader type shader
    glslang::TShader shader(shaderLanguage);

    // Put our shader source code in a string array
    const char* shaderSourceCode_array[1];
    shaderSourceCode_array[0] = pShader;

    // Initialize Built In Resources
    TBuiltInResource builtInResources;
    initBuiltInResources(builtInResources);

    // Set shader string in above array as Tshader's string
    shader.setStrings(shaderSourceCode_array, 1);

    // Specify rules of shader compilation as per spv & vulkan rules
    EShMessages ruleMessages = (EShMessages)(
        EShMsgSpvRules | 
        EShMsgVulkanRules 
    );

    // parse the shader source code
    if(!shader.parse(&builtInResources, 100, false, ruleMessages)) {
        fprintf(fptr, "glsl2Spv(): shader.parse() Failed!.\n");
        fprintf(fptr, "glsl2Spv(): %s\n", shader.getInfoLog());
        fprintf(fptr, "glsl2Spv(): DebugLog %s\n", shader.getInfoDebugLog());
        return (False);
    } else {
        fprintf(fptr, "glsl2Spv(): shader.parse() Successful!.\n");
    }
    ...
    if(!program.link(ruleMessages)) {
        fprintf(fptr, "glsl2Spv(): program.link() Failed!.\n");
        fprintf(fptr, "glsl2Spv(): %s\n", program.getInfoLog());
        fprintf(fptr, "glsl2Spv(): DebugLog %s\n", program.getInfoDebugLog());
        return (False);
    } else {
        fprintf(fptr, "glsl2Spv(): program.link() Successful!.\n");
    }

    // And now this send this whole program to SPIR-V
    glslang::GlslangToSpv(*program.getIntermediate(shaderLanguage), spv);
    if(spv.size() == 0) {
        fprintf(fptr, "glsl2Spv(): glslang::GlslangToSpv() Failed!.\n");
        return (False);
    } else {
        fprintf(fptr, "glsl2Spv(): glslang::GlslangToSpv() Successful!.\n");
    }
...
EShLanguage findShaderLanguage(const VkShaderStageFlagBits shaderType) {
    ...
    switch(shaderType) {
        ...
        default:
            fprintf(fptr, "findShaderLanguage(): Invalid Shader Type!.\n");
            return (EShLangVertex); 
    }
}

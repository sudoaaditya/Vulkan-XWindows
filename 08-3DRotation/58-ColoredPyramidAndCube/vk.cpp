...
// Global Variables
const char* gpszAppName = "AMK_VulkanXWindowsApp : 3D Rotation - Colored Pyramid & Cube";
...
VertexData vertexData_pyramid_color;
// Position & Color for Cube
VertexData vertexData_cube_position;
VertexData vertexData_cube_color;
...
void uninitialize(void) {
    ...
    if(vertexData_pyramid_color.vkDeviceMemory) {
        vkFreeMemory(vkDevice, vertexData_pyramid_color.vkDeviceMemory, NULL);
        fprintf(fptr, "uninitialize(): vkFreeMemory() Succeed for Vertex Buffer for Color!\n");
        vertexData_pyramid_color.vkDeviceMemory = VK_NULL_HANDLE;
    }

    if(vertexData_pyramid_color.vkBuffer) {
        vkDestroyBuffer(vkDevice, vertexData_pyramid_color.vkBuffer, NULL);
        fprintf(fptr, "uninitialize(): vkDestroyBuffer() Succeed for Vertex Buffer for Color!\n");
        vertexData_pyramid_color.vkBuffer = VK_NULL_HANDLE;
    }
    ...
    if(vertexData_cube_color.vkDeviceMemory) {
        vkFreeMemory(vkDevice, vertexData_cube_color.vkDeviceMemory, NULL);
        fprintf(fptr, "uninitialize(): vkFreeMemory() Succeed for Vertex Buffer for Color!\n");
        vertexData_cube_color.vkDeviceMemory = VK_NULL_HANDLE;
    }

    if(vertexData_cube_color.vkBuffer) {
        vkDestroyBuffer(vkDevice, vertexData_cube_color.vkBuffer, NULL);
        fprintf(fptr, "uninitialize(): vkDestroyBuffer() Succeed for Vertex Buffer for Color!\n");
        vertexData_cube_color.vkBuffer = VK_NULL_HANDLE;
    }
...
VkResult createVertexBuffer(void) {
    ...
    memset((void*)&vertexData_pyramid_color, 0, sizeof(VertexData));
    ...
    // Setp 4
    vkResult = vkCreateBuffer(vkDevice, &vkBufferCreateInfo, NULL, &vertexData_pyramid_color.vkBuffer);
    if(vkResult != VK_SUCCESS) {
    ...
    vkGetBufferMemoryRequirements(vkDevice, vertexData_pyramid_color.vkBuffer, &vkMemoryRequirements);
    ...
    //Setp 9
    vkResult = vkAllocateMemory(vkDevice, &vkMemoryAllocateInfo, NULL, &vertexData_pyramid_color.vkDeviceMemory);
    if(vkResult != VK_SUCCESS) {
    ...
    // Step 10
    vkResult = vkBindBufferMemory(vkDevice, vertexData_pyramid_color.vkBuffer, vertexData_pyramid_color.vkDeviceMemory, 0);
    if(vkResult != VK_SUCCESS) {
    ...
    vkResult = vkMapMemory(
        vkDevice,
        vertexData_pyramid_color.vkDeviceMemory,
        0,
        vkMemoryAllocateInfo.allocationSize,
        0,
        &data
    ...
    // Step 13
    vkUnmapMemory(vkDevice, vertexData_pyramid_color.vkDeviceMemory);
    ...
    // VertexData for Triangle Color
    memset((void*)&vertexData_cube_color, 0, sizeof(VertexData));
    ...
    // Setp 4
    vkResult = vkCreateBuffer(vkDevice, &vkBufferCreateInfo, NULL, &vertexData_cube_color.vkBuffer);
    if(vkResult != VK_SUCCESS) {
    ...
    vkGetBufferMemoryRequirements(vkDevice, vertexData_cube_color.vkBuffer, &vkMemoryRequirements);
    ...
    //Setp 9
    vkResult = vkAllocateMemory(vkDevice, &vkMemoryAllocateInfo, NULL, &vertexData_cube_color.vkDeviceMemory);
    if(vkResult != VK_SUCCESS) {
    ...
    // Step 10
    vkResult = vkBindBufferMemory(vkDevice, vertexData_cube_color.vkBuffer, vertexData_cube_color.vkDeviceMemory, 0);
    if(vkResult != VK_SUCCESS) {
    ...
    vkResult = vkMapMemory(
        vkDevice,
        vertexData_cube_color.vkDeviceMemory,
        0,
        vkMemoryAllocateInfo.allocationSize,
        0,
        &data
    ...
    // Step 13
    vkUnmapMemory(vkDevice, vertexData_cube_color.vkDeviceMemory);
...
VkResult createDescriptorPool(void) {
    ...
    vkDescriptorPoolSize.descriptorCount = 2; // we have two uniform buffer
    ...
    vkDescriptorPoolCreateInfo.maxSets = 2; // we have two descriptor set
...
VkResult createDescriptorSet(void) {
    ...
    vkWriteDescriptorSet.dstSet = vkDescriptorSet_pyramid; // this is the descriptor set we want to update
    ...
    vkWriteDescriptorSet.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    vkWriteDescriptorSet.pNext = NULL;
    vkWriteDescriptorSet.dstSet = vkDescriptorSet_cube; // this is the descriptor set we want to update
    vkWriteDescriptorSet.dstArrayElement = 0; // we have only one descriptor set, so array element is 0
    vkWriteDescriptorSet.descriptorCount = 1; // we are only gonna write one descriptor set
    vkWriteDescriptorSet.descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER; 
    vkWriteDescriptorSet.pBufferInfo = &vkDescriptorBufferInfo;
...
VkResult buildCommandBuffers(void) {
    ...
    for(uint32_t i = 0; i < swapchainImageCount; i++) {
        ...
        vkCmdBindVertexBuffers(
            vkCommandBuffer_array[i], 
            AMK_ATTRIBUTE_COLOR, 1,
            &vertexData_pyramid_color.vkBuffer,
        ...
        vkCmdBindVertexBuffers(
            vkCommandBuffer_array[i], 
            AMK_ATTRIBUTE_COLOR, 1,
            &vertexData_cube_color.vkBuffer,
            vkDeviceSize_offset_color_array
        );
